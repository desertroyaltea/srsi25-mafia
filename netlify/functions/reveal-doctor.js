// netlify/functions/reveal-doctor.js
...
exports.handler = async (event, context) => {
    ...
    try {
        ...
        const canRevealCol = playerHeaders.indexOf('DoctorCanRevealSelf');
        const revealedCol = playerHeaders.indexOf('RevealedTeammates');

        // 2. Find the Doctor, verify their ability, and get their current revealed list
        ...
        // 3. Find all eligible teammates (Alive, Doctor, not self, not already revealed)
        ...
        for (let i = 0; i < players.length; i++) {
            ...
            if (
                ...
                player[roleCol] === 'Doctor' &&
                playerId !== doctorPlayerId &&
        ...
        console.log(`Doctor ${doctorPlayerId} is revealing teammate ${randomTeammate.id} (${randomTeammate.name})`);
        ...
        const requests = [
            { // Update RevealedTeammates list
                range: `Players!${String.fromCharCode(65 + revealedCol)}${doctorPlayerRowIndex}`,
                values: [[newRevealedList]]
            },
            { // Set DoctorCanRevealSelf ability to FALSE (one-time use)
                range: `Players!${String.fromCharCode(65 + canRevealCol)}${doctorPlayerRowIndex}`,
                values: [['FALSE']]
            }
        ];
        ...
        return {
            ...
            body: JSON.stringify({ message: `A fellow Doctor has been revealed to you: ${randomTeammate.name}.` }),
        };

    } catch (error) {
        console.error('Error in reveal-doctor function:', error);
        return {
            statusCode: 500,
